...
import { stopBot } from "../../../../facebook-auto/lib/bot";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, status: await stopBot(ownerId) });
  } catch (error) {
    if (error instanceof FacebookAutoAuthError) {
      return NextResponse.json({ ok: false, message: error.message }, { status: error.status });
    }
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : "Không thể dừng bot." },
      { status: 400 }
    );
  }
}
